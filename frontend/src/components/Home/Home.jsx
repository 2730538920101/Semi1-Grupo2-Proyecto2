...
export default function Home({ user, setUser }) {
          ...
          if (res.data.success === true) {
            ...
            localStorage.setItem(
              ...
              JSON.stringify({
                ...
                EMAIL: user.EMAIL,
                DPI: editUser.DPI,
                APP_PASSWORD: editUser.APP_PASSWORD,
                PICTURE: res.data.PICTURE,
              })
            );
  ...
  const translate = (message) => {
    alert(message)
    axios.post("/user/translate", { message: message })
        .then((res) => {
            alert(res.data.en); // Mostrar la traducción en inglés, o cualquier otro idioma que prefieras
        })
        .catch((error) => {
            alert("Error al traducir el mensaje");
        });
};
            ...
            <>
              <MenuContainer>
                ...
                <MenuRight onClick={handleMenuToggle}>
                  <Image
                    src={process.env.REACT_APP_REQUEST_S3_URL + user.PICTURE}
                    alt="Imagen"
                    id="image"
                  />
                  <TextContainer>
                        ...
                        <ContentContainer>
                          ...
                          <ButtonsContainer>
                            {tipo ? (
                              ...
                              <Button1 onClick={capture}>Tomar foto</Button1>
                            )}
                            <Button0 onClick={handleChangeMode}>
                              {tipo
                                ? "Usar camara"
                                : "Usar imagen del dispositivo"}
                            </Button0>
                          </ButtonsContainer>
                          <ButtonsContainer>
                      ...
                      {filteredPosts.map((post, index) => (
                        <Post key={index}>
                          <LeftPost>
                            ...
                            <LeftImage
                              src={
                                process.env.REACT_APP_REQUEST_S3_URL +
                                post.IMAGE_POST
                              }
                          ...
                          <RightPost id={`rightPost-${index}`}>
                            <RightTop>
                              ...
                              <PostData>
                                ...
                                >
                                  <PostDate>
                                    Fecha: {post.APP_POST_DATE}
                                  </PostDate>
                                  <TranslateIcon
                                    onClick={() =>
                                      translate(post.APP_POST_DESCRIPTION)
                                    }
                                  />
                                </PostMeta>
                              </PostData>
                              ...
                              <CommentContainer key={index}>
                                <InputComment>
                                  <InputField
                                    ...
                                    onChange={(e) =>
                                      handleNewCommentChange(index, e)
                                    }
                                  />
                                  <SendButton
                                    onClick={() => doNewComment(index)}
                                  >
                                    Enviar
                                  </SendButton>
                                </InputComment>
                              ...
                              {post.COMENTS.map((comment, index) => (
                                <>
                                  {comment.APP_COMENT_ID !== null && (
                                    <CommentContainer key={index}>
                                      <CommentText>
                                        {comment.FULL_NAME_USER_COMENT}:{" "}
                                        {comment.COMENT}
                                      </CommentText>
                                      <CommentMeta>
                                        <PostDate>
                                          Fecha: {comment.APP_COMENT_DATE}
                                        </PostDate>
                                        <TranslateIcon
                                          onClick={() =>
                                            translate(post.APP_POST_DESCRIPTION)
                ...
                <RightContainer>
                  <Tabs>
                    <TabButton
                      ...
                      style={{
                        border:
                          activeTab === "Tab1" ? "2px solid white" : "none",
                      }}
                    >
                      <IoMdContacts /> Amigos
                    </TabButton>
                    <TabButton
                      ...
                      style={{
                        border:
                          activeTab === "Tab2" ? "2px solid white" : "none",
                      }}
                    >
                    ...
                    {chatID !== -1 ? (
                      <TabButton
                        ...
                        style={{
                          border:
                            activeTab === "Tab3" ? "2px solid white" : "none",
                        }}
                      >
                        ...
                        {listAmigos.map((item, index) => {
                          if (
                            ...
                            return (
                              <ListItem
                                key={index}
                                onClick={() => setChat(item.ID_FRIEND)}
                              >
                                <ItemImage
                                  src={
                                    process.env.REACT_APP_REQUEST_S3_URL +
                                ...
                                <ListItem key={index}>
                                  ...
                                  {item.APP_FRIEND_STATUS === "Esperando" && (
                                    <ContactState>
                                      <Button
                                        onClick={() =>
                                          friendrequestaccept(item.OTHER_ID)
                                        }
                                      >
                                        <MdOutlineCheckCircle />
                                      </Button>
                                      <Button
                                        onClick={() =>
                                          friendrequestreject(item.OTHER_ID)
                                        }
                                      >
                                        <MdOutlineCancel />
                                      </Button>
                                  ...
                                  {item.APP_FRIEND_STATUS === "Enviar" && (
                                    <ContactState
                                      onClick={() =>
                                        sendFriendRequest(item.OTHER_ID)
                                      }
                                    >
                                      <Button>
                                        <BsFillPersonPlusFill />
